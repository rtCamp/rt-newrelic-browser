...
if ( ! defined( 'RTP_RESOURCE_VERSION' ) ) {
	define( 'RTP_RESOURCE_VERSION', filemtime( __FILE__ ) );
}
...
if ( ! class_exists( 'Rt_Newrelic' ) ) {
	...
	class Rt_Newrelic {
		...
		public function rtp_relic_load_js() {
			wp_enqueue_script( 'rtp_relic_js', plugins_url( '/js/rtp-relic.js', __FILE__ ), array( 'jquery' ), RTP_RESOURCE_VERSION, true );
			wp_enqueue_script( 'jquery-ui-dialog' );

			wp_enqueue_style( 'rtp_relic_css', plugins_url( '/css/rtp-new-relic.css', __FILE__ ), array(), RTP_RESOURCE_VERSION );
			wp_enqueue_style( 'rtp_relic_ui_css', plugins_url( '/css/jquery-ui.css', __FILE__ ), array(), RTP_RESOURCE_VERSION );
		}
		...
		public function rtp_relic_validate_form( $relic_user_data ) {
			...
			if ( 'rtp-get-browser' === $form_name ) {
				...
				} elseif ( ( ! preg_match( '/^[a-zA-Z ]*$/', $relic_user_data['relic-first-name'] ) ) || ( ! preg_match( '/^[a-zA-Z ]*$/', $relic_user_data['relic-last-name'] ) ) ) {
					$relic_valid         = false;
		...
		public function rtp_relic_options_validate( $input ) {
			if ( isset( $_POST['submit_new_relic'] ) && wp_verify_nonce( sanitize_key( $_POST['submit_new_relic'] ), 'relic_options_nonce_action' ) ) {
				...
				if ( ! $rtp_relic_form_validated['valid'] ) {
					...
					if ( isset( $_POST['rtp-relic-form-name'] ) && 'rtp-select-browser' === sanitize_text_field( wp_unslash( $_POST['rtp-relic-form-name'] ) ) ) {
						...
						if ( ! $rtp_account_details_duplicate ) {
							 ...
							 * start of API 1
							 */
							$relic_account_name  = ( isset( $_POST['relic-account-name'] ) ) ? sanitize_text_field( wp_unslash( $_POST['relic-account-name'] ) ) : '';
							$relic_account_email = ( isset( $_POST['relic-account-email'] ) ) ? sanitize_email( wp_unslash( $_POST['relic-account-email'] ) ) : '';
							$relic_first_name    = ( isset( $_POST['relic-first-name'] ) ) ? sanitize_text_field( wp_unslash( $_POST['relic-first-name'] ) ) : '';
							$relic_last_name     = ( isset( $_POST['relic-last-name'] ) ) ? sanitize_text_field( wp_unslash( $_POST['relic-last-name'] ) ) : '';
							...
							} elseif ( isset( $json_data->api_key ) ) {
